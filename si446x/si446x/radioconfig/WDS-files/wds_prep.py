#!/usr/bin/env python
#
# There exists two copies of this file.   One in the TagNet repository:
# gh:MamMark/TagNet/si446x/si446x/radioconfig/WDS-files and one in the
# gh:MamMark/mm/tos/chips/si446x/WDS-files repositories.
#
# There really should be one, but where should it live.  If two copies
# which is the master?  They need to be kept in sync until this gets sorted
# out.
...
<WDS> above refers to the root/WDS_files directory that is the output
location of the WDS program.

wds_prep.py output also includes:
 - for each source tree directory found in 'WDS_files'
   - ./wds_config.c,   API to access specified config info
   - ./wds_config.h,   config info from source tree in condensed form,
                  includes radio_config.h and si446x_patch.h.
 - the wds_config api provides control for selecting different configurations
   from the collection.

wds_radio_configs is a list of tuples representing each of the
configuration sources. Each tuple consists of a byte array containing
configuration pstrings and a null terminated byte array for the name
...
const uint8_t *const *wds_config_list() {
    return wds_radio_configs;
}

const uint8_t *wds_default_name() {
    return wds_radio_configs[wds_default_override+1];
}

const uint8_t *wds_default_config() {
...
uint8_t const *wds_config_select(uint8_t *cname) {
    ...
    // if no name specified, return current default.
    if (!cname)
        return wds_radio_configs[wds_default_override];
...
def write_global_config(dirlist):
    ...
    with open('wds_configs.h','w+') as output:
        output.write(comment_code)
        output.write('\n')
        output.write('#ifndef __WDS_CONFIGS_H__\n')
        output.write('#define __WDS_CONFIGS_H__\n')
        output.write('\n')
        output.write(typedef_code)
        output.write('\n')
        output.write(extern_code)
        output.write('\n#endif /* __WDS_CONFIGS_H__ */\n')
